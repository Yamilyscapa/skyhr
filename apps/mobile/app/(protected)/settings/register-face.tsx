...
import {
  ...
  timingConfig,
} from "@/modules/biometrics/config";
...
export default function RegisterFaceScreen() {
  ...
  const extractLiveness = useCallback((response: any) => {
    ...
    const spoofFlagRaw =
      liveness?.spoofFlag ?? liveness?.spoof_flag ?? liveness?.isSpoof;
    const spoofFlag = Boolean(spoofFlagRaw);

    return { score, spoofFlag };
  }, []);

  const registerFaceOnce = useCallback(
    async (image: CapturedImage) => {
      ...
      try {
        ...
        if (spoofFlag || (score !== null && score < MIN_LIVENESS_SCORE)) {
          const scoreMessage =
            score !== null ? ` (puntaje ${Math.round(score)})` : "";
          ...
          Alert.alert("Necesitamos una imagen más nítida", message, [
        ...
        Alert.alert("No pudimos registrar tu rostro", message, [
          { text: "Reintentar", onPress: resetCaptureState },
        ]);
      } finally {
  ...
  useFaceDetection(cameraRef, {
    enabled:
      hasPermission && !isCaptureDone && !hasRegisteredFace && hasOrganization,
  ...
  const handleManualScanPress = useCallback(async () => {
    ...
    try {
      ...
      if (!photo?.uri) {
        throw new Error("No se pudo capturar una imagen valida");
      }

      await registerFaceOnce({
        uri: photo.uri,
        width: photo.width ?? 0,
  ...
  }, [isCaptureDone, isSubmitting, registerFaceOnce, resetCaptureState]);
    ...
    return (
      <View style={styles.permissionContainer}>
        ...
        <Button
          ...
          accessibilityLabel="Otorgar permiso"
          accessibilityHint="Solicita acceso a la camara para registrar tu rostro"
        >
          Otorgar permiso
        </Button>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View
        ...
        onLayout={(event) =>
          setInstructionsHeight(event.nativeEvent.layout.height)
        }
      >
        <Text style={styles.instructionsTitle}>Alinea tu rostro</Text>
        <Text style={styles.instructionsSubtitle}>{instructionsText}</Text>
      </View>
      ...
      <View style={[styles.fallbackContainer, { bottom: insets.bottom + 20 }]}>
        <Button
          style={styles.scanAssistButton}
          onPress={handleManualScanPress}
          disabled={isSubmitting || isCaptureDone}
          accessibilityLabel="Escanear"
          accessibilityHint="Toma una foto para registrar tu rostro si el escaneo automatico falla"
        >
          Escanear
        </Button>
      </View>
