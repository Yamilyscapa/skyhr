...
export default function BiometricsScanner() {
    ...
    async (image: CapturedImage) => {
      if (!hasOrganization) {
        Alert.alert(
          ...
          "Debes pertenecer a una organización para registrar asistencia.",
        );
        return;
      }
      ...
      if (!latitude || !longitude) {
        Alert.alert(
          "Error de ubicación",
          "No se pudo obtener tu ubicación. Asegúrate de haber otorgado permisos de ubicación.",
        );
        return;
      }
      ...
      try {
        ...
        if (response.data.spoof_flag) {
          ...
          if (score !== null) {
            console.log(
              `Check-in successful. Liveness score: ${score.toFixed(1)}`,
            );
            ...
            Alert.alert("Asistencia registrada", feedbackMessage, [
              {
                text: "OK",
                onPress: () => router.replace("/(protected)/(tabs)"),
              },
  ...
  const handleManualScanPress = useCallback(async () => {
    if (!cameraRef.current || isSubmitting || isCaptureDone) {
      return;
    }
    ...
    try {
      ...
      if (!photo?.uri || !photo.base64) {
        Alert.alert(
          "Error de escaneo",
          "No pudimos capturar una imagen valida.",
        );
        return;
      }

      await handleDetectionComplete({
        uri: photo.uri,
        width: photo.width ?? 0,
      ...
      console.error("Manual biometric capture failed:", error);
      Alert.alert(
        "Error de escaneo",
        "No pudimos procesar la captura. Intenta nuevamente.",
      );
    }
  }, [handleDetectionComplete, isCaptureDone, isSubmitting]);
  ...
  return (
    <>
      ...
      <CameraView ref={cameraRef} facing="front" style={styles.container} />
      ...
      <View style={[styles.fallbackContainer, { bottom: insets.bottom + 20 }]}>
        <Button
          ...
          accessibilityHint="Toma una foto para registrar asistencia si el escaneo automatico falla"
        >
          Escanear
        </Button>
      </View>
