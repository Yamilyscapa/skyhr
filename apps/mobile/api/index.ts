...
class Api {
  ...
  public async validateQR(qrData: string) {
    return this.post("attendance/qr/validate", { qr_data: qrData }) as Promise<{
      data: { location_id: string; organization_id: string };
    }>;
  }

  public async validateVisitorQR(qrData: string) {
    const response = (await this.post("visitors/qr/validate", {
      qr_data: qrData,
    ...
    const mappedData = {
      ...
      approvedByUserId:
        visitorData.approved_by_user_id || visitorData.approvedByUserId,
      approvedAt: visitorData.approved_at || visitorData.approvedAt,
      createdByUserId:
        visitorData.created_by_user_id || visitorData.createdByUserId,
  ...
  public async getVisitor(id: string) {
    const response = (await this.get(`visitors/${id}`)) as {
      data: any;
      message: string;
    };
    const visitorData = response.data;
  ...
  public async watchModeCheckIn(data: {
    ...
    return this.post(
      "attendance/watch-mode/check-in",
      data,
    ) as Promise<WatchModeCheckInResponse>;
  }
  ...
  public async getOrganizationGeofences(
    ...
    return this.get(
      `geofence/get-by-organization?${searchParams.toString()}`,
    ) as Promise<{
      message: string;
  ...
  public async getInvitationStatus(organizationId: string, email: string) {
    if (!organizationId || !email) {
      throw new Error(
        "Se requieren el ID de la organización y el correo para consultar la invitación.",
      );
    }
    const query = new URLSearchParams({ email });
    return this.get(
      `organizations/${organizationId}/invitations/by-email?${query.toString()}`,
    );
  }
   ...
   */
  public async getTodayAttendanceEvent(
    userId: string,
  ): Promise<{ status: number; data: any }> {
    try {
      const headers = await this.getAuthHeaders();
      const response = await this.fetchWithTimeout(
        `${this.baseUrl}/attendance/today/${userId}`,
        {
          headers,
          credentials: "omit",
        },
  ...
  public async deleteAccount() {
    return this.delete("users/me") as Promise<{ message: string }>;
  }
   ...
   */
  public async getPermissions(filters?: {
    status?: string;
    userId?: string;
    page?: number;
    pageSize?: number;
   ...
   */
  public async getPendingPermissions(filters?: {
    page?: number;
    pageSize?: number;
  }) {
    const queryParams = new URLSearchParams();
    ...
    const queryString = queryParams.toString();
    const url = queryString
      ? `permissions/pending?${queryString}`
      : "permissions/pending";
    return this.get(url) as Promise<{
      data: Array<{
        id: string;
   ...
   */
  public async uploadPermissionDocuments(
    id: string,
